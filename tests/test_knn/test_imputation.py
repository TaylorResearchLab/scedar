...
class TestFeatureImputation(object):
    ...
    def run_test_impute_features(self, nprocs):
        ...
        # lookup
        np.testing.assert_equal(fmp._res_lut[(1, 1, 0.5, 1, np.median)][0],
                                res_sdml[0]._x)
        np.testing.assert_equal(fmp._res_lut[(3, 1, 1.5, 1, np.median)][0],
                                res_sdml[1]._x)
        np.testing.assert_equal(fmp._res_lut[(5, 3, 0.5, 5, np.median)][0],
                                res_sdml[2]._x)
        ...
        assert len(fmp._res_lut) == 3
        np.testing.assert_equal(fmp._sdm._d, d)
        np.testing.assert_equal(fmp._sdm.x, self.tx)
