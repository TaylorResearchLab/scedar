...
class TestFeatureKNNPickUp(object):
    ...
    def run_test_knn_pickup_features(self, nprocs):
        ...
        # lookup
        np.testing.assert_equal(fkp._res_lut[(1, 1, 0.5, 1)][0]._x,
                                res_sdml[0]._x)
        np.testing.assert_equal(fkp._res_lut[(3, 1, 1.5, 1)][0]._x,
                                res_sdml[1]._x)
        np.testing.assert_equal(fkp._res_lut[(5, 3, 0.5, 5)][0]._x,
                                res_sdml[2]._x)
